...
class NAG():
    def __init__(self, nag_params, image_params, rn, resume, num_classes, resolution, datasets=None, is_ft=False):
        ...
        #get custom data loader that maps indices to images in the dataset
    ...
    def init_models_weights(self):
        if self.nag_params.z_init == "rndm":
            print(f"init_models_weights(self): init rndm")
            self.netZ.apply(model.weights_init)
            self.netZ.set_maps_from_loader()
        if self.nag_params.z_init == "cube":
            print(f"init_models_weights(self): init cube")
            self.netZ.apply(model.weights_init)
            self.netZ.cube_init()
        if self.nag_params.z_init == "resnet":
            print(f"init_models_weights(self): init resnet")
            self.netZ.apply(model.weights_init)
    ...
    def train_long(self, train_data, unlabeled_train_data, test_data_transductive, opt_params, resolution,
        ...
        if self.BEN_COMMENTS: print("\n[BEN] Class NAG: Starting train_long() \n")
        ...
        #distance metrics and more
        if self.BEN_COMMENTS: print("\n[BEN] Creating distance metric objects\n")
        ...
        #add transductive learning stuff
        if is_transductive:
            if self.BEN_COMMENTS:  print("\n[BEN] loading transductive training\n")
        ...
        while epoch < opt_params.epochs:
            ...
            #set optimizers for both models G and z
            if self.BEN_COMMENTS: print("\n[BEN] set G and Z optimizers: scaling G lr by {}\n".format(opt_params.factor))
            ...
            #adding in the classifier optimizer
            ...
            if unlabeled_data_loader is not None:
                if self.BEN_COMMENTS:  print("\n[BEN] starting unlabeled data forward pass {}\n")
            ...
            if is_transductive:
                if self.BEN_COMMENTS:  print("\n[BEN] starting do transductive forward pass {}\n")
            ...
            if epoch % vis_epochs == 0:
                if self.BEN_COMMENTS:  print("\n[BEN] Visualizing models\n")
                # try:
                self.visualize_no_module(epoch, self.aug_param,"the commented param is not used") #train_data.data)
                # except Exception as e:
                #     print(e)
    ...
    def train_epoch(self, data_loader, epoch, optim_Z, optim_G, optim_D, scheduler, gamma, is_transductive=False):
        ...
        if self.BEN_COMMENTS:  print("\n[BEN] starting epoch in TRAIN\n")
        if self.BEN_COMMENTS:  print("\n[BEN] TRAIN data loader length: {}\n".format(len(data_loader)))
        ...
        for i, batch in enumerate(data_loader):
            ...
            if self.BEN_COMMENTS and self.BENCOUNT == 0:  
                print("\n[BEN] TRAIN data loader Batch Image shape: {}\n".format(images.shape))

            if self.BEN_COMMENTS and self.BENCOUNT == 0:  
                print("\n[BEN] TRAIN data loader Batch: {}\n".format(batch))
                self.BENCOUNT +=1
            ...
            if self.BEN_COMMENTS:  print("\n[BEN] TRAIN generated noise code: {} shape:{}\n".format(code,code.shape))
            # normed_code = code.norm(2, 1).detach().unsqueeze(1).expand_as(code)
            # code = code.div(normed_code)

            zi = self.netZ(idx) #retrieve the sphere vecto zi given the index of the dataset
            if self.noise2niose:
...
class NAGTrainer():
    def __init__(self, dataset, nag_params, rn, resume=False, num_classes=10):
        ...
        #[*** NEW PRINT ***]nag_params: NAGParams(nz=512, force_l2=False, is_pixel=True, z_init='rndm', is_classifier=True, disc_net='conv', loss='ce', data_name='cifar-100', noise_proj=True, shot=10)
        # , rn: cifar-100_my_test_10unsuprvised_pixel_classifier_conv_tr_fs_10_ce_noise_proj_rndm_70_0.001_0.7_0.5_gamma0.5, dataset <class 'list'>
        print("\n\n\n[*** NEW PRINT ***]nag_params: {}, rn: {}, dataset {}".format(nag_params,rn,type(dataset)))

        print(f"\n\n\n[DEBUG] data name={nag_params.data_name}, data res = {resolution}")
        ...
        print("[NEW CODE] image_params:{}".format(self.image_params))
